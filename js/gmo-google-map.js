...
var GMOGoogleMap = function(element, pos, zoom, infoCont) {
    this.base_url = 'https://maps.google.com/maps?';
    this.display(element, pos, zoom, infoCont);
}

GMOGoogleMap.prototype.display = function(element, pos, zoom, infoCont) {
...
$('.gmogooglemap').each(function(){
    ...
    if ($(element).data('lat') && $(element).data('lng')) {
        ...
        new GMOGoogleMap(element, pos, zoom, infoCont);
        ...
        GMaps.geocode({
            ...
            callback: function(results, status) {
                if (status == 'OK') {
                    ...
                    new GMOGoogleMap(element, pos, zoom, $(element).html());
                }
            }
        });
        ...
        GMaps.geocode({
            ...
            callback: function(results, status) {
                if (status == 'OK') {
                    var pos = results[0].geometry.location;
                    new GMOGoogleMap(element, pos, zoom, $(element).text());
                }
            }
        });
    }
